...
export class RecipeService {
    ...
    async addRecipe(recipe: Partial<Recipe>): Promise<Recipe> {
        const response = await apiClient.post(API_ENDPOINTS.RECIPE.CREATE_RECIPE, recipe);
        return response.data;
    }

    // Generate recipe using AI
    async generateAIRecipe(payload: GenerateAIRecipePayload): Promise<any> {
        const response = await apiClient.post(API_ENDPOINTS.RECIPE.GENERATE_AI, payload);
        return response.data;
    }

    // Generate custom recipe
    async generateCustomRecipe(payload: GenerateCustomRecipePayload): Promise<any> {
        const response = await apiClient.post(API_ENDPOINTS.RECIPE.SEARCH_CUSTOM, payload);
        return response.data;
    }

    // Update an existing recipe
    async updateRecipe(id: string, data: any): Promise<Recipe> {
        const response = await apiClient.patch(API_ENDPOINTS.RECIPE.UPDATE_RECIPE(id), data);
        return response.data;
    }
    ...
    async updateCookingStatus(
        ...
        const response = await apiClient.patch(API_ENDPOINTS.RECIPE.UPDATE_STATUS(id), {
            status,
        });
        return response.data;
    }
