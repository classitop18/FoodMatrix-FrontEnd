...
export const useSendInvitation = () => {
    ...
    return useMutation({
        mutationFn: (payload: { email: string; accountId: string; proposedRole?: string }) =>
...
export const useApproveInvitation = () => {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: ({ invitationId, role, accountId }: { invitationId: string, role: string, accountId: string }) =>
            InvitationService.approveInvitation(invitationId, role),
        onSuccess: (data, variables) => {
            queryClient.invalidateQueries({ queryKey: ["account-invitations", variables.accountId] });
            queryClient.invalidateQueries({ queryKey: ["members", { accountId: variables.accountId }] });
            toast({
        ...
        onError: (error: any) => {
            toast({
                ...
                description: error.response?.data?.message || "Failed to approve invitation",
                variant: "destructive",
            })
        }
    });
...
export const useRejectInvitation = () => {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: ({ invitationId, reason, accountId }: { invitationId: string, reason?: string, accountId: string }) =>
        ...
        onError: (error: any) => {
            toast({
                ...
                description: error.response?.data?.message || "Failed to reject invitation",
                variant: "destructive",
            })
        }
    });
...
export const useCancelInvitation = () => {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: ({ invitationId, accountId }: { invitationId: string, accountId: string }) =>
