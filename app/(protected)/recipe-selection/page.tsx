...
import { getComplementaryItems } from "@/lib/recipe-utils";
import RecipeSelectionCard, { Slot } from "@/components/meal-planning/recipe-selection-card";
...
export default function RecipeSelection() {
    ...
    const [generatedRecipies, setGeneratedRecipies] = useState<Record<string, { recipes: Recipe[] }>>({});
    const [generatedCustomRecipies, setGeneratedCustomRecipies] = useState<Record<string, { recipes: Recipe[] }>>({});
                ...
                (["breakfast", "lunch", "dinner"] as const).forEach((mealTime) => {
                    ...
                    if (["cook-home", "quick-simple", "meal-prep"].includes(mealType)) {
                        slots.push({
                            ...
                            type: mealType === "cook-home" ? "Cook at home" : mealType === "quick-simple" ? "Quick/Simple" : "Meal prep",
                        });
                    }
                });
            });
    ...
    const generateRecipePayload = (slotKey: string, key: keyof RecipeEntry, value: any) => {
    ...
    const generateRecipe = async (slot: Slot) => {
        ...
        if (!payload?.cuisine) {
            toast({ title: "Select Cuisine", description: "Please select a cuisine first.", variant: "destructive" });
            return;
        }

        setIsSlotProcessing(prev => ({ ...prev, [slotKey]: { ...prev[slotKey], isRecipeLoading: true } }));
        ...
        try {
            const response = await generateRecipeWithAi.mutateAsync({
                ...
                usePantryItems: payload.pantryOption === 'can-use' || payload.pantryOption === 'only-pantry',
            ...
            const recipes = (response.recipes || []).map((r: any, idx: number) => ({
                ...
                complementaryItems: getComplementaryItems(payload.cuisine!, slot.meal),
                isAIGenerated: true
            }));
            ...
            toast({ title: "Recipes Generated!", description: `Found ${recipes.length} recipes.` });

        } catch (error) {
            toast({ title: "Error", description: "Failed to generate recipes.", variant: "destructive" });
        } finally {
            setIsSlotProcessing(prev => ({ ...prev, [slotKey]: { ...prev[slotKey], isRecipeLoading: false } }));
        }
    };
    ...
    const generateCustomRecipe = async (slot: Slot) => {
        ...
        setIsSlotProcessing(prev => ({ ...prev, [slotKey]: { ...prev[slotKey], isCustomRecipeLoading: true } }));
        ...
        try {
            ...
            toast({ title: "Error", description: "Could not find custom recipe.", variant: "destructive" });
        } finally {
            setIsSlotProcessing(prev => ({ ...prev, [slotKey]: { ...prev[slotKey], isCustomRecipeLoading: false } }));
        }
    };
    ...
    const handleGenerateShoppingList = () => {
        ...
        localStorage.setItem('foodmatrix-selected-recipes', JSON.stringify({
            recipeIds,
            ...
            <div className="container mx-auto px-4 md:px-6 relative z-10 py-12 max-w-8xl">
                ...
                <div className="text-center mb-12 animate-in fade-in slide-in-from-top-4 duration-700">
                    ...
                    <p className="text-lg text-gray-500 font-medium max-w-2xl mx-auto leading-relaxed">
                        Personalize each meal with our AI chef. Choose cuisines, specific dishes, or let us surprise you.
                    </p>
                </div>
                    ...
                    <div className="xl:col-span-3 space-y-8">
                        {mealSlots.length === 0 ? (
                            <div className="text-center py-20 bg-white rounded-2xl border border-dashed border-gray-200">
                                ...
                                <h3 className="text-lg font-bold text-gray-900">No Meals Planned for Cooking</h3>
                                <p className="text-gray-500">Go back to Meal Planning to schedule some home-cooked meals.</p>
                                <Button onClick={() => router.push('/meal-planning')} variant="outline" className="mt-4">
                                    Go to Meal Planning
                                </Button>
                            </div>
                        ) : (
                            mealSlots.map((slot) => {
                                const slotKey = `${slot.date}-${slot.meal}`;
                                return (
                                    <div key={slotKey} className="animate-in slide-in-from-bottom-4 duration-500">
                                        <RecipeSelectionCard
                                            slot={slot}
                                            ...
                                            customRecipes={generatedCustomRecipies[slotKey]?.recipes || []}
                            ...
                            <Card className="border-0 shadow-[0_2px_20px_-4px_rgba(0,0,0,0.08)] bg-white/80 backdrop-blur-md overflow-hidden">
                                ...
                                <CardContent className="space-y-6">
                                    <div className="bg-emerald-50 rounded-xl p-4 border border-emerald-100">
                                        <span className="text-xs font-bold text-emerald-600 uppercase tracking-wider block mb-1">Total Estimated Cost</span>
                                    ...
                                    <ScrollArea className="h-[300px] pr-4">
                                        <div className="space-y-3">
                                            {Object.entries(selections).map(([key, id]) => {
                                                ...
                                                return (
                                                    <div key={key} className="flex items-start gap-3 p-3 bg-white rounded-lg border border-gray-100 shadow-sm relative group">
                                                        <button
                                                            onClick={() => {
                                                                const newSel = { ...selections };
                                                                delete newSel[key];
                                                                setSelections(newSel);
                                                                setSelectedRecipes(prev => prev.filter(x => x !== id));
                                                            }}
                                                        ...
                                                        <div className="overflow-hidden">
                                                            ...
                                                            <p className="text-xs text-gray-500 uppercase tracking-wider font-semibold">{key.split('-')[1]}</p>
                                                        </div>
                                                    </div>
                                                )
                                            })}
