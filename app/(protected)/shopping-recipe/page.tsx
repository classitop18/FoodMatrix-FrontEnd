...
export default function ShoppingRecipe() {
    ...
    const handleAddItem = () => {
        ...
        toast({ title: "Item Added", description: `${newItem.name} added to list` });
    };
    ...
    const handleGenerateFinalList = () => {
        ...
        localStorage.setItem('finalShoppingList', JSON.stringify({
                                                    ...
                                                    <div className="space-y-4">
                                                        <div className="space-y-2">
                                                            ...
                                                            <div className="flex items-center border border-gray-200 rounded-lg p-1 w-full max-w-[150px]">
                                                                <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => setAddItemQuantity(Math.max(1, addItemQuantity - 1))}>
                    ...
                    <div className="lg:max-w-[550px] w-full bg-[#e9e2fe] rounded-lg overflow-hidden">
                        <div className="bg-gradient-to-r from-[var(--primary)] to-[var(--primary-light)] text-white p-4 flex justify-between items-center">
                            ...
                            <span className="text-sm font-normal">
                                {Object.values(plans).reduce((acc, day) => acc + Object.keys(day).length, 0)} Items
                            </span>
                        </div>
                                                            ...
                                                            <div className="flex gap-3">
                                                                <div className="h-16 w-16 shrink-0 rounded-md bg-gray-100 overflow-hidden">
                                                                    {recipe.image || (recipe as any).imageUrl ? (
                                                                        <img src={getRecipeImageUrl(recipe.image || (recipe as any).imageUrl)!} alt={recipe.name} className="h-full w-full object-cover" />
                                                                    ) : <div className="h-full w-full flex items-center justify-center"><Apple className="text-gray-300 size-6" /></div>}
                                                                </div>
                                                                <div className="flex-1 min-w-0">
                                                                    <div className="flex justify-between items-start">
                                                                        <h3 className="font-bold text-gray-900 line-clamp-1 text-sm">{recipe.name}</h3>
                                                                        <span className="text-[10px] font-bold uppercase tracking-wider text-[var(--primary)] bg-[#F2EEFF] px-1.5 py-0.5 rounded-sm">{mealType}</span>
                                                                    </div>
                ...
                <div className="rounded-lg bg-[#e9e2fe] mt-5 p-4">
                    <div className="flex gap-3 items-center font-bold justify-between flex-wrap">
                        <div className="flex flex-col">
                            ...
                            <span className="text-sm font-medium text-black/60">You have {allIngredients.length} items from {allIngredients.length > 0 ? Object.keys(plans).length + 1 : 0} sources</span>
                        </div>
