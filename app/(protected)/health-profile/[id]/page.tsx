...
export default function HealthProfilePage() {
  ...
  const {
    data: memberHealth,
    isLoading: isHealthProfileFetching,
    refetch: refetchHealth,
  } = useGetHealthProfile(memberId!);

  // Fetch member data to check if user is invited
  ...
  // Disable if: member has userId (invited user) AND it's not the current user's profile
  const isInvitedUser = !!memberData?.data?.userId;
  const isOwnProfile = memberData?.data?.userId === currentUser?.data?.id;
  const isEditDisabled = isInvitedUser && !isOwnProfile;
    ...
    try {
      ...
      const transformedData = {
        ...
        targetWeight: formData.targetWeight
          ...
          : undefined,
        dailySodiumLimitMg: formData.dailySodiumLimitMg
          ? Number(formData.dailySodiumLimitMg)
          : undefined,
        dailyCarbLimitG: formData.dailyCarbLimitG
          ? Number(formData.dailyCarbLimitG)
          : undefined,
        dailyCalorieTarget: formData.dailyCalorieTarget
          ? Number(formData.dailyCalorieTarget)
          : undefined,
        dailyFiberTargetG: formData.dailyFiberTargetG
          ? Number(formData.dailyFiberTargetG)
          : undefined,
  ...
  if (isHealthProfileFetching || isMemberLoading) {
    return <Loader />;
  }
          ...
          <div className="flex items-center gap-3">
            {isEditing ? (
              ...
              <button
                ...
                className={`bg-[#7661d3] hover:bg-[#6952c2] text-white font-bold py-2.5 px-4 rounded-xl shadow-lg transition-all flex items-center gap-2 text-sm ${isEditDisabled
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-white/90 text-sm leading-relaxed">
                  Fill out the information below to help us provide personalized
                  meal recommendations, track your nutrition goals, and tailor
                  recipes to your dietary needs and preferences. You can always
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    >
                      ...
                      <option value="mostly_home">
                        Mostly Home Cooking
                      </option>
                      <option value="mixed">Mixed (Home & Dining Out)</option>
                      <option value="mostly_dining_out">Mostly Dining Out</option>
                    </select>
                  </div>
