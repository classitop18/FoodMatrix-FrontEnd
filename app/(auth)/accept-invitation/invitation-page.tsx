            ...
            try {
                ...
                if (isAuthenticated && user) {
                    if (user.email.toLowerCase() === details.email.toLowerCase()) {
                        ...
                        setStatus("error");
                        setErrorMessage(`This invitation was sent to ${details.email}, but you are signed in as ${user.email}. Please sign out and use the correct account.`);
                    }
                } else {
                ...
                setStatus("error");
                setErrorMessage(error.response?.data?.message || "Failed to process invitation. It might be expired or already used.");
            }
        };
                    ...
                    {invitationDetails ? (
                        <>
                            <p className="text-gray-600 mb-2 leading-relaxed">
                                Invitation for <span className="font-bold text-[#7661d3]">{invitationDetails.email}</span>
                            </p>
                            ...
                            <div className="space-y-3">
                                {invitationDetails.exists ? (
                                    <ThemeButton
                                        ...
                                        onClick={() => router.push(`/login?email=${encodeURIComponent(invitationDetails.email)}&returnUrl=${encodeURIComponent(`/accept-invitation?token=${token}`)}`)}
                                        label="Sign In to Accept"
                                    />
                                ) : (
                                    <ThemeButton
                                        className="w-full h-12 font-bold text-base"
                                        onClick={() => router.push(`/register?email=${encodeURIComponent(invitationDetails.email)}&readonlyEmail=true&returnUrl=${encodeURIComponent(`/accept-invitation?token=${token}`)}`)}
