...
const FOOD_CATEGORIES = {
    ...
    Vegetables: ["Broccoli", "Spinach", "Carrots", "Tomatoes", "Onions", "Garlic", "Bell Peppers"],
...
export function FoodPreferencesForm({
    ...
    const addCustomExclusion = () => {
        if (customExclusionInput.trim() && !customExclusions.includes(customExclusionInput.trim())) {
            onCustomExclusionsChange([...customExclusions, customExclusionInput.trim()]);
    ...
    const addCustomInclusion = () => {
        if (customInclusionInput.trim() && !customInclusions.includes(customInclusionInput.trim())) {
            onCustomInclusionsChange([...customInclusions, customInclusionInput.trim()]);
    ...
    const applyPreset = (presetName: string) => {
        const preset = PRESET_PREFERENCE_SETS[presetName as keyof typeof PRESET_PREFERENCE_SETS];
        if (preset) {
                ...
                {preferenceSets?.length > 0 && (
                    <div className="mt-3 flex flex-wrap gap-2">
                        {preferenceSets?.map((preset) => (
                            <Badge key={preset} variant="secondary" className="flex items-center gap-1 px-3 py-1 rounded-full">
                                {preset}
                                <button
                                    type="button"
                        ...
                        <div key={category} className="space-y-2">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {foods.map((food) => (
                                    <Badge
                                        key={food}
                                        variant={excludedFoods?.includes(food) ? "default" : "outline"}
                                        className={`cursor-pointer px-3 py-1 rounded-full transition-all ${excludedFoods?.includes(food)
