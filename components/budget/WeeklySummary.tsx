        ...
        <div className="bg-white rounded-2xl p-6 shadow-md border border-gray-200">
            ...
            <div className="flex items-center justify-between mb-5">
                <div>
                    ...
                    <p className="text-xs text-gray-400">
                        {(() => {
                            const [sYear, sMonth, sDay] = data.weekStart.split("-").map(Number);
                            const [eYear, eMonth, eDay] = data.weekEnd.split("-").map(Number);

                            return `${new Date(sYear, sMonth - 1, sDay).toLocaleDateString("en-US", {
                                day: "numeric",
                                month: "short",
                            })} – ${new Date(eYear, eMonth - 1, eDay).toLocaleDateString("en-US", {
                                day: "numeric",
                                month: "short",
                            })}`;
                        })()}
            ...
            <div className="grid grid-cols-7 gap-2">
                {data.days.map((day) => {
                    ...
                    const localDayDate = new Date(year, month - 1, d);
                    const isToday =
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-white/60">
                                            {(() => {
                                                const [year, month, day] = selectedDateToUpdate.date.split("-").map(Number);
