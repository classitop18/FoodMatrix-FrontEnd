...
import { PantryUpdateDialog } from "@/components/meal-planning/pantry-update-dialog";
...
export default function RecipeSelectionCard(props: RecipeSelectionCardProps) {
  ...
  const [recipePreferences, setRecipePreferences] = useState<
    Record<string, RecipePreference>
  >({});

  // State for pantry update dialog
  const [cookingRecipe, setCookingRecipe] = useState<any | null>(null);
  const [isPantryUpdateOpen, setIsPantryUpdateOpen] = useState(false);
  ...
  const handlePantryUpdateSuccess = () => {
    ...
    toast({
      title: "Bon Appétit! 🍽️",
      description: "Recipe marked as cooked. Score +2",
    });
  };
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button
                      ...
                      className="w-7 h-7 flex items-center justify-center rounded-md transition-all bg-gray-100 text-gray-500 hover:bg-blue-50 hover:text-blue-600"
                    >
  ...
  return (
    <div className="p-4 sm:p-6 bg-gradient-to-br from-blue-50/30 via-indigo-50/20 to-purple-50/30">
        ...
        <CardHeader className="pb-5 border-b bg-gradient-to-r from-blue-50 via-indigo-50 to-purple-50">
          <CardTitle className="text-base sm:text-lg">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              ...
              <Badge
                variant="secondary"
                className="self-start sm:self-center capitalize text-sm font-semibold bg-indigo-100 text-indigo-700 border-indigo-200"
              >
                {slot.type}
              </Badge>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="space-y-1.5">
                <MultiSelectDropdown
                  ...
                  onChange={(updated) =>
                    generateRecipePayload(slotKey, "members", updated)
                  }
                />
              </div>
            </div>
            ...
            <div className="space-y-2.5 pt-2">
              ...
              >
                {[
                  ...
                  <div
                    key={option.value}
                    className="flex items-center gap-2 px-3.5 py-2 rounded-lg border border-indigo-200 bg-white hover:bg-indigo-50 hover:border-indigo-300 transition-all cursor-pointer"
                  >
                    <RadioGroupItem value={option.value} id={option.value} />
                    <Label
                      htmlFor={option.value}
                      className="text-xs font-semibold cursor-pointer text-gray-700"
                    >
                      {option.label}
                    </Label>
                  </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between pb-3 border-b">
                ...
                <Badge className="text-xs font-semibold bg-emerald-100 text-emerald-700 border-emerald-200">
                  ...
                  {customRecipes.length === 1 ? "Result" : "Results"}
                </Badge>
              </div>
              <div className="grid grid-cols-1 gap-3">
      ...
      {cookingRecipe && (
        <PantryUpdateDialog
          isOpen={isPantryUpdateOpen}
          onClose={() => setIsPantryUpdateOpen(false)}
          recipeName={cookingRecipe.name}
          ingredients={cookingRecipe.ingredients || []}
          onSuccess={handlePantryUpdateSuccess}
        />
      )}
    </div>
  );
...
// Simple internal helper for tooltip content to avoid issues
function TooltipContentSide({ children }: { children: React.ReactNode }) {
  return (
