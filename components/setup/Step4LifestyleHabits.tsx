...
export default function Step4LifestyleHabits({ form }: Step4Props) {
  return (
    ...
    >
      ...
      <div className="bg-white border-2 border-[#7661d3]/20 shadow-2xl p-6 rounded-2xl  shadow-lg border border-[#7661d3]/20">
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-6">
          <div>
            ...
            <Select
              onValueChange={(value) =>
                form.setValue("cookingSkill", value as any)
              }
            >
              <SelectTrigger
                data-testid="select-cooking-skill"
              ...
              <SelectContent>
                <SelectItem value="beginner">
                  👶 Beginner (basic meals, simple recipes)
                </SelectItem>
                <SelectItem value="moderate">
                  👨‍🍳 Moderate (comfortable with most recipes)
                </SelectItem>
                <SelectItem value="advanced">
                  ⭐ Advanced (complex recipes, techniques)
                </SelectItem>
              </SelectContent>
            </Select>
            {form.formState.errors.cookingSkill && (
              <p className="text-sm text-red-500 mt-1">
                {String(form.formState.errors.cookingSkill.message)}
              </p>
            )}
          </div>
          ...
          <div>
            ...
            <Select
              onValueChange={(value) =>
                form.setValue("cookingFrequency", value as any)
              }
            >
              <SelectTrigger
                data-testid="select-cooking-frequency"
                className="h-12 border-2 border-[#7661d3]/30 focus:border-[#7661d3] rounded-xl bg-white"
              >
                <SelectValue placeholder="How often do you cook?" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="mostly_home">
                  🏠 Mostly at home (5+ meals/week)
                </SelectItem>
                <SelectItem value="mixed">🔄 Mixed (2-4 meals/week)</SelectItem>
                <SelectItem value="mostly_dining_out">
                  🍽️ Mostly dining out (0-1 meals/week)
                </SelectItem>
              </SelectContent>
            </Select>
            {form.formState.errors.cookingFrequency && (
              <p className="text-sm text-red-500 mt-1">
                {String(form.formState.errors.cookingFrequency.message)}
              </p>
            )}
          </div>
        </div>
        ...
        <div>
          ...
          <Select
            onValueChange={(value) =>
              form.setValue("budgetFlexibility", value as any)
            }
          >
            <SelectTrigger
              data-testid="select-budget-flexibility"
              className="h-12 border-2 border-[#7661d3]/30 focus:border-[#7661d3] rounded-xl bg-white"
            >
            ...
            <SelectContent>
              ...
              <SelectItem value="moderate">
                ⚖️ Moderate (small overages for health trade-offs)
              </SelectItem>
              <SelectItem value="flexible">
                💰 Flexible (health over budget when needed)
              </SelectItem>
            </SelectContent>
          </Select>
          {form.formState.errors.budgetFlexibility && (
            <p className="text-sm text-red-500 mt-1">
              {String(form.formState.errors.budgetFlexibility.message)}
            </p>
          )}
        </div>
      </div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <label className="flex items-start space-x-4 p-5 border-2 border-[#7661d3]/20 rounded-xl hover:bg-[#F3F0FD] cursor-pointer transition-all duration-300 hover:border-[#7661d3] hover:shadow-md group">
            <Checkbox
              ...
              onCheckedChange={(checked) =>
                form.setValue("hasDeepFreezer", !!checked)
              }
            ...
            <div>
              <div className="flex items-center mb-1">
                ...
                <span className="font-bold text-[#3d326d] group-hover:text-[#7661d3]">
                  Have Deep Freezer
                </span>
              </div>
              <p className="text-sm text-gray-600">
                Can buy and store bulk frozen foods
              </p>
            </div>
          </label>
          ...
          <label className="flex items-start space-x-4 p-5 border-2 border-[#7dab4f]/20 rounded-xl hover:bg-[#E8F5E0] cursor-pointer transition-all duration-300 hover:border-[#7dab4f] hover:shadow-md group">
            <Checkbox
              ...
              onCheckedChange={(checked) =>
                form.setValue("shopsDaily", !!checked)
              }
            ...
            <div>
              <div className="flex items-center mb-1">
                <span className="text-2xl mr-2">🛒</span>
                <span className="font-bold text-[#3d326d] group-hover:text-[#7dab4f]">
                  Shop Daily for Fresh Items
                </span>
              </div>
              <p className="text-sm text-gray-600">
                Prefer fresh ingredients over bulk shopping
              </p>
            </div>
          </label>
        </div>
      ...
      <motion.div
        className="bg-white border-2 border-[#7661d3]/20 shadow-2xl rounded-2xl  p-6 shadow-lg"
        initial={{ scale: 0.95 }}
        animate={{ scale: 1 }}
        ...
        <div className="flex items-start space-x-4">
          ...
          <div>
            ...
            <p className="text-gray-700 leading-relaxed">
              Your health profile will help{" "}
              <span className="font-bold text-[#7661d3]">FoodMatrix</span>{" "}
              provide personalized food recommendations, budget optimizations,
              and health-aligned meal planning while staying within your food
              budget.
            </p>
